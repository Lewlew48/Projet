...
<div class="container-fluid">
    ...
    // Ajouter la condition libelle_Ma à la requête SQL
    if (isset($_POST['libelle']) && $_POST['libelle'] == 'on') {
        $sql .= " AND m.libelle_Ma LIKE :query";
        $param = '%' . $query . '%';
        $counter++;
    }

    // Ajouter la condition attributaire_Ma à la requête SQL
    if (isset($_POST['attributaire']) && $_POST['attributaire'] == 'on') {
        if ($counter > 0) {
            $sql .= " OR m.attributaire_Ma LIKE :query";
        } else {
            $sql .= " AND m.attributaire_Ma LIKE :query";
            $counter++;
        }
        $param = '%' . $query . '%';
    }
    ...
    $date = $mois . '/' . $annee;
    $marches = fetchMarches($mois, $annee, $sql, $counter, $param);
            ...
            <div class="row">
                ...
                <div class="col-12">
                    ...
                    <form class="menu-selection" style="text-decoration: none;float: right;"
                          ...
                          method="post">
                        <!-- Type de l'envoi -->
                        <input type="hidden" name="type" value="Marches">
                        <!-- Nom du fichier -->
