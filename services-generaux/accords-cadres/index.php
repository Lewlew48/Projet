...
<div class="container-fluid">
    ...
    // Ajouter la condition libelle_Ac à la requête SQL
    if (isset($_POST['libelle']) && $_POST['libelle'] == 'on') {
        $sql .= " AND a.libelle_Ac LIKE :query";
        $param = '%' . $query . '%';
        $counter++;
    }

    // Ajouter la condition attributaire_Ac à la requête SQL
    if (isset($_POST['attributaire']) && $_POST['attributaire'] == 'on') {
        if ($counter > 0) {
            $sql .= " OR a.attributaire_Ac LIKE :query";
        } else {
            $sql .= " AND a.attributaire_Ac LIKE :query";
            $counter++;
        }
        $param = '%' . $query . '%';
    }
                        ...
                        if (empty($accords)) {
                            ...
                            for ($i = ($limite - 50);
                                ...
                                if (isset($accords[$i]) && $accords[$i] != NULL) {
                                    ...
                                    echo '<tr>';
                                    echo '<td>' . $accord['dateCreation'][2] . $accord['dateCreation'][3] . '-' . sprintf("%04d", $accord['id']) . '</td>'; // Affichage de la date de création avec l'ID de l'accord
                                    echo '<td>' . $accord['dateCreation'] . '</td>'; // Affichage de la date de création
                                    echo '<td>' . $accord['libelle'] . '</td>'; // Affichage du libellé
                                    echo '<td>' . $accord['direction'] . '</td>'; // Affichage de la direction
                                    echo '<td>' . $accord['instructeur'] . '</td>'; // Affichage de l'instructeur
                                    echo '<td>' . $accord['attributaire'] . '</td>'; // Affichage de l'attributaire
